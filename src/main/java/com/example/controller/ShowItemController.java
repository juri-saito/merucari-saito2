package com.example.controller;


import java.util.List;

import javax.servlet.http.HttpSession;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.data.domain.Page;
import org.springframework.stereotype.Controller;
import org.springframework.ui.Model;
import org.springframework.web.bind.annotation.ModelAttribute;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.SessionAttributes;

import com.example.domain.Category;
import com.example.domain.Item;
import com.example.form.SearchItemForm;
import com.example.service.ItemService;
import com.example.service.PullDownService;

/**
 * 商品情報表示関連機能の処理の制御を行うコントローラ
 * @author juri.saito
 *
 */
@Controller
@RequestMapping("/top")
public class ShowItemController {
	
	@Autowired
	private ItemService itemServise;
	
	@Autowired
	private PullDownService searchItemService;
	
	@Autowired
	private HttpSession session;
	
	@ModelAttribute
	public SearchItemForm setupForm() {
		return new SearchItemForm();
	}
	/**
	 * 全商品一覧画面を表示
	 * @param 出力したいページ番号
	 * @return 全商品一覧画面
	 */
	@RequestMapping("")
	public String showAllItems(Model model, SearchItemForm form, Integer page) {
		
		session.setAttribute("searchItemForm", form);
		
//		1ページに全商品を表示
//		List<Item> itemList = itemServise.findAll();
//		model.addAttribute("itemList", itemList);
		
		//①ページング機能追加
		if(page == null) {
			//ページ番号の指定が無い場合は1ページ目を表示させる
			page = 1;
		}
		//TODO 検索機能
		
		//表示させたいページ番号を渡し、1ページに表示させる商品リストを取得
		Page<Item> itemPage = itemServise.showListPaging(page);
		model.addAttribute("itemPage", itemPage);
		
		//表示させたいページのページ番号をスコープに格納
		//TODO list.htmlに1ページ目と最終ページのときはprevボタンとnextボタンが消えるようにjquery書く
		int currentNumber = itemPage.getNumber();
		model.addAttribute("currentNumber", currentNumber);
		
		//表示させたいページの前のページ番号をスコープに格納
		model.addAttribute("prevPagePath", "/top?page=" + (currentNumber-1));
		//表示させたいページの次のページ番号をスコープに格納
		model.addAttribute("nextPagePath", "/top?page=" + (currentNumber+1));
		
		
		//総ページ数をスコープに格納
		int totalPages = itemPage.getTotalPages();
		model.addAttribute("totalPages", totalPages);

		//TODO ②オートコンプリート
		
		//③大カテゴリのリストをスコープに格納
		List<Category> parentList = searchItemService.findParent();
		model.addAttribute("parentList", parentList);
			
		//④商品一覧画面出力
		return "list.html";
	}

		
}
